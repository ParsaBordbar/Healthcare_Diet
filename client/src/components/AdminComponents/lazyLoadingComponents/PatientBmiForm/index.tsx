  ...
  return (
    <section className='flex flex-col bg-[var(--milky-white)] py-4 rounded-lg gap-4 shadow-md'>
        <div className='flex flex-col 2xl:flex-row justify-between text-[var(--text)] mt-3 items-start gap-4 2xl:gap-0 2xl:items-center'>
          ...
          <p className='flex items-center text-[var(--new-green)] text-lg font-semibold w-fit px-6 2xl:p-2.5 rounded-full'>BMI:{" "} {bmi}</p>
        </div>
        ...
        <p className='2xl:px-10 px-6  flex items-center 2xl:my-2 text-xl text-[var(--tittle-box)]'> شماره تماس:  {" "}  {phoneNumber}</p>
    </section>
  )
}
